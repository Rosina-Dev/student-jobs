...
function renderJobs(){
 ...
 list.innerHTML=visible.map(j=>`<article class="job-card" tabindex="0" data-id="${j.id}" aria-label="View ${j.title} at ${j.company}"><div class="company-logo ${j.tone}">${j.initials}</div><div>${j.publicListing?`<span class="public-label">Public listing · not employer verified</span>`:`<span class="sample-label">Sample job - prototype only.</span>`}<h3>${j.title}</h3><p>${j.company} · ${j.location}</p><div class="job-meta">${j.publicListing?`<span class="public-source">${j.sourceLabel} · Checked ${j.checked}</span>`:`<span class="verified">✓ Verified example</span>`}<span>${j.time}</span><span>${j.age}</span></div></div><div class="pay"><strong>${j.pay}</strong><span>${j.hours}</span></div><button class="save-job ${savedJobs.has(j.id)?'saved':''}" data-save="${j.id}" aria-label="${savedJobs.has(j.id)?'Remove from saved jobs':'Save job'}" aria-pressed="${savedJobs.has(j.id)}">${savedJobs.has(j.id)?'♥':'♡'}</button></article>`).join('');
...
function openJob(id){const j=jobs.find(x=>x.id===id);document.querySelector('#modal-content').innerHTML=`${j.publicListing?`<span class="public-label">Public listing · not employer verified</span>`:`<span class="sample-label">Sample job - prototype only.</span>`}<div class="modal-head"><div class="company-logo ${j.tone}">${j.initials}</div><div><p class="eyebrow">${j.company}</p><h2>${j.title}</h2>${j.publicListing?`<span class="public-source">${j.sourceLabel} · Checked ${j.checked}</span>`:`<span class="verified">✓ Verified example</span>`}</div></div><p>${j.summary}</p><div class="detail-grid"><div><span>Pay</span><strong>${j.pay}</strong></div><div><span>Weekly hours</span><strong>${j.hours}</strong></div><div><span>Schedule</span><strong>${j.time}</strong></div><div><span>Minimum age</span><strong>${j.age}</strong></div></div>${j.publicListing?`<div class="modal-note"><strong>Why this listing is shown</strong><br>${j.sourceNote}</div>`:`<div class="modal-note"><strong>Why this job is shown to you</strong><br>Your age, location, and weekly availability match this role. The posting passed an initial safety review.</div>`}<div class="modal-actions">${j.publicListing?`<a class="primary-btn source-link" href="${j.sourceUrl}" target="_blank" rel="noopener">View original listing</a>`:`<button class="primary-btn apply-btn">Start short application</button>`}</div>`;modal.showModal();if(!j.publicListing)document.querySelector('.apply-btn').addEventListener('click',()=>{modal.close();toast('Application started — no résumé required.')})}
